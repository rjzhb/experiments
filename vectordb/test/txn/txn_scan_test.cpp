...
TEST(TxnScanTest, DISABLED_ScanTest) {  // NOLINT
  ...
  auto table_info = vdbms->catalog_->CreateTable(nullptr, "maintable", *schema);
  ...
  vdbms->txn_manager_->UpdateVersionLink(rid1, VersionUndoLink{prev_log_1}, nullptr);
  auto rid2 = *table_info->table_->InsertTuple(TupleMeta{txn3->GetReadTs(), false},
                                               Tuple{{Int(3), DoubleNull(), BoolNull()}, schema.get()});
  vdbms->txn_manager_->UpdateVersionLink(rid2, VersionUndoLink{prev_log_2}, nullptr);
  auto rid3 = *table_info->table_->InsertTuple(TupleMeta{txn4->GetReadTs(), true},
                                               Tuple{{IntNull(), DoubleNull(), BoolNull()}, schema.get()});
  vdbms->txn_manager_->UpdateVersionLink(rid3, VersionUndoLink{prev_log_4}, nullptr);
  auto rid4 = *table_info->table_->InsertTuple(TupleMeta{txn3->GetTransactionTempTs(), true},
                                               Tuple{{IntNull(), DoubleNull(), BoolNull()}, schema.get()});
  vdbms->txn_manager_->UpdateVersionLink(rid4, VersionUndoLink{prev_log_5}, nullptr);

  TxnMgrDbg("before verify scan", vdbms->txn_manager_.get(), table_info, table_info->table_.get());

  auto query = "SELECT * FROM maintable";
  fmt::println(stderr, "A: Verify txn0");
  WithTxn(txn0, QueryShowResult(*vdbms, _var, _txn, query, AnyResult{}));
  ...
  // WithTxn(txn2, QueryHideResult(*vdbms, _var, _txn, query, IntResult{})); // <- you will need to fill in the answer
  // fmt::println(stderr, "D: Verify txn3");
  // WithTxn(txn3, QueryHideResult(*vdbms, _var, _txn, query, IntResult{})); // <- you will need to fill in the answer
  // fmt::println(stderr, "E: Verify txn4");
  // WithTxn(txn4, QueryHideResult(*vdbms, _var, _txn, query, IntResult{})); // <- you will need to fill in the answer
  // fmt::println(stderr, "F: Verify txn5");
  // WithTxn(txn5, QueryHideResult(*vdbms, _var, _txn, query, IntResult{})); // <- you will need to fill in the answer
}
