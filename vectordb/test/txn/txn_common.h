...
template <typename T>
void Query(vdbmsInstance &instance, const std::string &txn_var_name, Transaction *txn, const std::string &query,
...
template <typename T>
void QueryHideResult(vdbmsInstance &instance, const std::string &txn_var_name, Transaction *txn,
...
template <typename T>
void QueryShowResult(vdbmsInstance &instance, const std::string &txn_var_name, Transaction *txn,
                     const std::string &query, const std::vector<std::vector<T>> &expected_rows) {
  Query(instance, txn_var_name, txn, query, expected_rows, true);
}

void CheckUndoLogNum(vdbmsInstance &instance, const std::string &txn_var_name, Transaction *txn, size_t expected_num) {
...
void CheckUndoLogColumn(vdbmsInstance &instance, const std::string &txn_var_name, Transaction *txn,
...
void Execute(vdbmsInstance &instance, const std::string &query, bool log = true) {
  if (log) {
...
auto TableHeapEntry(vdbmsInstance &instance, TableInfo *table_info) -> size_t {
...
void TableHeapEntryNoMoreThan(vdbmsInstance &instance, TableInfo *table_info, size_t upper_limit) {
...
void ExecuteTxn(vdbmsInstance &instance, const std::string &txn_var_name, Transaction *txn, const std::string &query) {
...
auto BeginTxn(vdbmsInstance &instance, const std::string &txn_var_name,
...
auto BeginTxnSerializable(vdbmsInstance &instance, const std::string &txn_var_name) -> Transaction * {
  return BeginTxn(instance, txn_var_name, IsolationLevel::SERIALIZABLE);
}

const bool EXPECT_FAIL = true;

auto CommitTxn(vdbmsInstance &instance, const std::string &txn_var_name, Transaction *txn, bool expect_fail = false) {
  if (txn->GetTransactionState() != TransactionState::RUNNING) {
    fmt::println(stderr, "txn not running");
    std::terminate();
  }
...
auto AbortTxn(vdbmsInstance &instance, const std::string &txn_var_name, Transaction *txn) {
  if (txn->GetTransactionState() != TransactionState::RUNNING &&
...
auto CheckTainted(vdbmsInstance &instance, const std::string &txn_var_name, Transaction *txn) {
...
auto CommitTaintedTxn(vdbmsInstance &instance, const std::string &txn_var_name, Transaction *txn) {
...
void ExecuteTxnTainted(vdbmsInstance &instance, const std::string &txn_var_name, Transaction *txn,
...
void GarbageCollection(vdbmsInstance &instance) {
  fmt::println(stderr, "- {}", Header("garbage_collection"));
  instance.txn_manager_->GarbageCollection();
}

void EnsureTxnGCed(vdbmsInstance &instance, const std::string &txn_var_name, txn_id_t txn_id) {
...
void EnsureTxnExists(vdbmsInstance &instance, const std::string &txn_var_name, txn_id_t txn_id) {
...
void BumpCommitTs(vdbmsInstance &instance, int by = 1) {
...
void EnsureIndexScan(vdbmsInstance &instance) {
...
template <typename T>
void QueryIndex(vdbmsInstance &instance, const std::string &txn_var_name, Transaction *txn, const std::string &query,
