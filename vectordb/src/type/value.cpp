...
Value::Value(TypeId type, int16_t i) : Value(type) {
  switch (type) {
	...
	case TypeId::BIGINT:value_.bigint_ = i;
	  size_.len_ = (value_.bigint_ == vdbms_INT64_NULL ? vdbms_VALUE_NULL : 0);
	  break;
	case TypeId::TIMESTAMP:value_.timestamp_ = i;
	  size_.len_ = (value_.timestamp_ == vdbms_TIMESTAMP_NULL ? vdbms_VALUE_NULL : 0);
	  break;
	default:throw Exception(ExceptionType::INCOMPATIBLE_TYPE, "Invalid Type for two-byte Value constructor");
  }
}
...
Value::Value(TypeId type, int32_t i) : Value(type) {
  switch (type) {
	...
	case TypeId::TIMESTAMP:value_.timestamp_ = i;
	  size_.len_ = (value_.timestamp_ == vdbms_TIMESTAMP_NULL ? vdbms_VALUE_NULL : 0);
	  break;
	default:throw Exception(ExceptionType::INCOMPATIBLE_TYPE, "Invalid Type for integer_ Value constructor");
  }
}
...
Value::Value(TypeId type, int64_t i) : Value(type) {
  switch (type) {
	...
	case TypeId::TIMESTAMP:value_.timestamp_ = i;
	  size_.len_ = (value_.timestamp_ == vdbms_TIMESTAMP_NULL ? vdbms_VALUE_NULL : 0);
	  break;
	default:throw Exception(ExceptionType::INCOMPATIBLE_TYPE, "Invalid Type for eight-byte Value constructor");
  }
}
...
Value::Value(TypeId type, uint64_t i) : Value(type) {
  switch (type) {
	...
	case TypeId::TIMESTAMP:value_.timestamp_ = i;
	  size_.len_ = (value_.timestamp_ == vdbms_TIMESTAMP_NULL ? vdbms_VALUE_NULL : 0);
	  break;
	default:throw Exception(ExceptionType::INCOMPATIBLE_TYPE, "Invalid Type for timestamp_ Value constructor");
  }
}
...
Value::Value(TypeId type, double d) : Value(type) {
  switch (type) {
	case TypeId::DECIMAL:value_.decimal_ = d;
	  size_.len_ = (value_.decimal_ == vdbms_DECIMAL_NULL ? vdbms_VALUE_NULL : 0);
	  break;
	default:throw Exception(ExceptionType::INCOMPATIBLE_TYPE, "Invalid Type for double Value constructor");
  }
}
...
Value::Value(TypeId type, float f) : Value(type) {
  switch (type) {
	case TypeId::DECIMAL:value_.decimal_ = f;
	  size_.len_ = (value_.decimal_ == vdbms_DECIMAL_NULL ? vdbms_VALUE_NULL : 0);
	  break;
	default:throw Exception(ExceptionType::INCOMPATIBLE_TYPE, "Invalid Type for float value constructor");
  }
}
...
Value::Value(TypeId type, const char *data, uint32_t len, bool manage_data) : Value(type) {
  switch (type) {
	...
	case TypeId::VECTOR:
	  ...
	  break;
	default:throw Exception(ExceptionType::INCOMPATIBLE_TYPE, "Invalid Type  for variable-length Value constructor");
  }
}
...
auto Value::CheckComparable(const Value &o) const -> bool {
  switch (GetTypeId()) {
	case TypeId::BOOLEAN:return (o.GetTypeId() == TypeId::BOOLEAN || o.GetTypeId() == TypeId::VARCHAR);
	case TypeId::TINYINT:
	...
	case TypeId::DECIMAL:
	  switch (o.GetTypeId()) {
		...
		case TypeId::BIGINT:
		case TypeId::DECIMAL:
		case TypeId::VARCHAR:return true;
		default:break;
	  }  // SWITCH
	  break;
	case TypeId::VARCHAR:
...
auto Value::CheckInteger() const -> bool {
  switch (GetTypeId()) {
	...
	case TypeId::SMALLINT:
	case TypeId::INTEGER:
	case TypeId::BIGINT:return true;
	default:break;
  }
  return false;
}
...
auto Value::GetColumn() const -> Column {
  switch (GetTypeId()) {
	...
	default:return Column{"<val>", GetTypeId()};
  }
}
