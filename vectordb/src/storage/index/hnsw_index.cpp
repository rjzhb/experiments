...
-> std::vector<size_t> {
  ...
  // 使用最大堆来维护最小的m个元素，这里的最大堆实际上存储的是最小的元素，堆顶是最大的那个最小元素
  std::priority_queue<std::pair<double, size_t>, std::vector<std::pair<double, size_t>>, std::less<>> min_heap;
//  std::vector<double> distances(vertex_ids.size());
  for (const auto vert : vertex_ids) {
	auto dist = ComputeDistance(vertices[vert], vec, dist_fn);
	if (min_heap.size() < m) {
...
auto NSW::SearchLayerParallel(const std::vector<double> &base_vector,
							  size_t limit,
							  const std::vector<size_t> &entry_points)
-> std::vector<size_t> {
  vdbms_ASSERT(limit > 0, "limit > 0");
  ...
  std::priority_queue<std::pair<double, size_t>, std::vector<std::pair<double, size_t>>, std::greater<>> explore_q;
  std::priority_queue<std::pair<double, size_t>, std::vector<std::pair<double, size_t>>, std::less<>> result_set;
  std::vector<std::future<void>> distance_futures;
  ...
  while (!explore_q.empty()) {
	...
	for (const auto &neighbor : edges_[vertex]) {
	  if (visited.find(neighbor) == visited.end()) {
		futures.push_back(std::async(std::launch::async, [&, neighbor]() {
		  double dist = ComputeDistance(vertices_[neighbor], base_vector, dist_fn_);
		  return std::make_pair(dist, neighbor);
		}));
		visited.emplace(neighbor);
	  }
	}
...
auto SelectNeighbors(const std::vector<double> &vec, const std::vector<size_t> &vertex_ids,
					 const std::vector<std::vector<double>> &vertices, size_t m, VectorExpressionType dist_fn)
-> std::vector<size_t> {
  return PARALLEL_ENABLED ? SelectNeighborsParallel(vec, vertex_ids, vertices, m, dist_fn) : SelectNeighborsNormal(vec,
																												   vertex_ids,
																												   vertices,
																												   m,
																												   dist_fn);
...
-> std::vector<size_t> {
  return PARALLEL_ENABLED ? SearchLayerNormal(base_vector, limit, entry_points) : SearchLayerNormal(base_vector,
																									limit,
																									entry_points);
}
